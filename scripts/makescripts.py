...
for s in spacing:
    ...
    with open(filename_clustering, "w") as f:
        ...
        if slurm:
            ...
            f.write(
                f"python {clustering_dir}/run_old.py --filename {test_dir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}.root --output {clustering_outdir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}_old.csv --sugar {sugarFile}  --sepR True --n_boxes {nboxes} --boxes_per_R {n_div_theta*n_div_Z}\n")
            f.write(
                f"python {clustering_dir}/run_up.py --filename {test_dir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}.root --output {clustering_outdir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}_up.csv --sugar {sugarFile}  --sepR True --n_boxes {nboxes} --boxes_per_R {n_div_theta*n_div_Z} --primary alpha\n")
            f.write(f"rm {clustering_outdir}/*.dat\n")
            f.write(
                f"python {clustering_dir}/run_up.py --filename {test_dir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}.root --output {clustering_outdir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}_up.csv --sugar {sugarFile}  --sepR True --n_boxes {nboxes} --boxes_per_R {n_div_theta*n_div_Z} --primary e-\n")
            f.write(f"rm {clustering_outdir}/*.dat\n")
            f.write(
                f"python {clustering_dir}/run_up.py --filename {test_dir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}.root --output {clustering_outdir}/out_AtDNA_{n_string}_spacing_{s_string}um_{seed}_up.csv --sugar {sugarFile}  --sepR True --n_boxes {nboxes} --boxes_per_R {n_div_theta*n_div_Z} --primary gamma\n")
            f.write(f"rm {clustering_outdir}/*.dat\n")
...
print(f"tmux new-session -s tat_{n_string}_{seed}\n 'source {filename_runscript}' ")
print("Done.")
