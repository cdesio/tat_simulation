...
for seed in seeds:
    filename = (
        f"./results/{folder}/runPhotonScript_{seed}.sh"  # to be run from folder created
    )
    if slurm:
    ...
    with open(filename, "w") as f:
        ...
        if not slurm:
            f.write(f"chmod 755 runSecondaryScript_{seed}.sh\n")
            f.write(f"./runSecondaryScript_{seed}.sh\n")
        if slurm:
            f.write(f"sbatch runSecondaryScript_{seed}.sh\n")

    filename = f"results/{folder}/runSecondaryScript_{seed}.sh"  # to be run from folder created
    with open(filename, "w") as f:
        f.write("#!/bin/bash --login\n")
        if slurm:
            f.write(f"#SBATCH --job-name=secondary_{seed}\n")
            f.write(f"#SBATCH --output=secondary_{seed}.out.%J\n")
            f.write(f"#SBATCH --error=secondary_{seed}.err.%J\n")
            # maximum job time in D-HH:MM
            f.write(f"#SBATCH --time={timeSec}\n")
            f.write("#SBATCH --nodes=1\n")
            f.write("#SBATCH -p short\n")
            f.write(f"#SBATCH --ntasks-per-node={numThread}\n")
            f.write(f"#SBATCH --mem {memSec}GB\n")
            f.write("\n")
            f.write("module load apps/geant/4.11.0.0\n")

        # secondary particle simulation
        f.write(
            f"time ../../../../simulation/build/rbe -mac secondary.in -PS photon_{seed}.bin -out secondary_{seed} -sugar {sugarFile} -histone {histoneFile} -seed {seed}\n"
        )

        if slurm:
            f.write("module load apps/root/6.26.00\n")

        combinedFilename = f"secondary_{seed}.root"

        output = "hadd " + combinedFilename

        for i in range(0, numThread):
            toCombine = f" secondary_{seed}_t{i}.root"
            output += toCombine

        f.write(output + "\n")

        for i in range(0, numThread):
            fileToDelete = f"secondary_{seed}_t{i}.root"
            f.write("rm {}\n".format(fileToDelete))

        if not slurm:
            f.write(f"chmod 755 clustering_{seed}.sh\n")
            f.write(f"./clustering_{seed}.sh\n")
        if slurm:
            f.write(f"sbatch clustering_{seed}.sh\n")

    filename = (
        f"./results/{folder}/clustering_{seed}.sh"  # to be run from folder created
    )
    ...
    with open(filename, "w") as f:
        ...
        if slurm:
            f.write(f"#SBATCH --job-name=clustering_{seed}\n")
            f.write(f"#SBATCH --output=clustering_{seed}.out.%J\n")
            f.write(f"#SBATCH --error=clustering_{seed}.err.%J\n")
            # maximum job time in D-HH:MM
        ...
        f.write(
            f"python ../../../../Clustering/run.py --filename secondary_{seed}.root --output photon_{seed}.csv --sugar {sugarFile} --filenamePhoton photon_{seed}.root \n"
        )
