...
def runClustering(
    ...
    fEMaxDamage: float,
    probIndirect: float,
    sugarPosFilename: str,
    shift: list = [0, 0, 0],
    filenamePhoton: Union[str, bool] = False,
):
    ...
    sugar0 = np.array(sugar0) + np.array(shift)
    sugar1 = np.array(sugar1) + np.array(shift)
    # print(sugar0, sugar1)
    ...
    for i in range(entryEdepNumber):
        ...
        if hasattr(
            tEdep, "PathLengthChromatin"
        ):  # older results do not have path length
        ...
        if hasattr(tEdep, "PrimaryKEEntrance"):  # older results do not have path length
            meanKEperEvent[tEdep.EventNo] = (
                tEdep.PrimaryKEEntrance + tEdep.PrimaryKEExit
            ) / 2
        else:
    ...
    if filenamePhoton:
        ...
        for key in cumulatedEnergyDep:
            if (mapping[key[0]], key[1], key[2]) in cumulatedEnergyDepPhoton:
                cumulatedEnergyDep[key] += cumulatedEnergyDepPhoton[
    ...
    for irow in range(nentries_indirect):
        ...
        if hasattr(input_tree_indirect, "copyNum"):
            ...
            if np.random.rand() <= probIndirect:
                eventsListIndirect.append(input_tree_indirect.EventNo)
                point = [
                    input_tree_indirect.x,
                    input_tree_indirect.y,
                    input_tree_indirect.z,
                ]
                c, s = getIndex(point, T0, T1)
                copyListIndirect.append(c)
                strandListIndirect.append(s)
        else:
            if (input_tree_indirect.DNAmolecule[0] == "D") and (
                input_tree_indirect.radical == "OH^0"
            ):
                if np.random.rand() <= probIndirect:
                    eventsListIndirect.append(input_tree_indirect.EventNo)
                    point = [
    ...
    with open(outputFilename, "w") as f:
        ...
        f.write(
            "EventNo,DoseGy,PathLength_nm,MeanKEMeV,TotalSBdirect,SSBdirect,cSSBdirect,DSBdirect,TotalSBindirect,SSBindirect,cSSBindirect,DSBindirect,TotalSBtotal,SSBtotal,cSSBtotal,DSBtotal\n"
        )
        for event in events:
            if event in eventsWithClusteringResults:
                text = (
                    str(
                        [
                            clusteringResults[eventsWithClusteringResults.index(event)][
                                1:
                            ]
                        ]
                    )
