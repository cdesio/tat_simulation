...
# def map_DNA_to_decay(ps_data: dict):
#      # build map to decay event number
#     mapping = {}  # photon event ID to simulation event ID dict
#     step1_evts = ps_data['PhotonEventID'].array(library='np')
#     step2_evts = ps_data['EventNo'].array(library='np')
...
# def map_decay_to_copyno(ps_data):
#     # build map to decay event number
#     mapping_copyNo = {}  # photon event ID to simulation event ID dict
#     step_1_evts = ps_data['PhotonEventID'].array(library='np')
#     copynos = ps_data['copyNo'].array(library='np')
...
# def map_PID_to_DNA(ps_data: dict, pid: int):
#     # # build map to decay event number
#     # mapping_PID = defaultdict(list)  # photon event ID to simulation event ID dict
#     # for PID, evt_step2 in zip(ps_data['particleID'], ps_data['EventNo']): #loop on nentries           
#     #     mapping_PID[PID].append(evt_step2) #save photon event number into key simulation event number
#     # # end mapping 
#     # return mapping_PID
#     return lambda pid: ps_data['EventNo'][ps_data['particleID']==pid]

def map_pid_idx(ps_data_pid, pid: int):
    if pid==-1:
        return np.where(ps_data_pid!=-1)[0]
    else:
        return np.where(ps_data_pid==pid)[0]
...
def calculateDose(eventEdep, chromatinVolume: float, keys_df):

    print("start calculate dose")
    evts_step2 = keys_df['step2'].to_numpy() 

    edep_J = eventEdep['edep_J'].array(library='numpy')
    edep_MeV = eventEdep['edep_MeV'].array(library='numpy')
...
def AccumulateEdep(direct, T0: cKDTree, T1: cKDTree, keys_df, out_path):
    ...
    meventNo = np.memmap(out_path+'_temp_mevt.dat', mode='w+', shape = eventNo.shape, dtype='int64')
    meventNo[:] = eventNo[:]
    del eventNo
    evts_step2 = keys_df['step2'].to_numpy()
    selected_direct = np.isin(meventNo, evts_step2) #selecting only events of PID
    #get x, y, z
    ...
    #change to evt_step1
    keys_map = keys_df.loc[keys_df['step2'].isin(evt2_filtered)].set_index('step2')
    ...
    evt1_cp_filtered['copy'] = results_filtered[:,1]  #evt ID (1), copyNo, strand, copy
    evt1_cp_filtered['edep'] = edep_filtered

    
    cumEdep = evt1_cp_filtered.groupby(['step1', 'copyNo', 'strand', 'copy', 'pid'],as_index=False)['edep'].sum()
...
def calcDirectDamage(cumulatedEnergyDep, fEMinDamage: float, fEMaxDamage: float):
    ...
    cumulatedEnergyDep['doesdamage'] = cumulatedEnergyDep['edep'].apply(check_edep)
...
def runClustering(filename_DNA: str, outputFilename: str, fEMinDamage: float, fEMaxDamage: float, probIndirect: float, sugarFname: str, primaryParticle: str = None, filenamePhoton: Union[str, bool] = False,  separate_r=False, n_boxes = 3200, boxes_per_R = 800):
    ...
    ps_data = ufile["output/PS_data"]
    ps_data_pid = ps_data['step1_particleID'].array(library='np')

    evt_id_pid = map_pid_idx(ps_data_pid, primaryParticleID)
    evts_pid = ps_data_pid[evt_id_pid]
    evts_step1 = ps_data['step1_eventID'].array(library='np')[evt_id_pid]
    evts_step2 = ps_data['step2_eventID'].array(library='np')[evt_id_pid]
    copyNo_pid = ps_data['step1_copyNo'].array(library='np')[evt_id_pid]
    evt_keys = np.vstack([evts_step1, evts_step2, copyNo_pid, evts_pid]).T

    #make DataFrame with step1, step2, copyNo and PID
    keys_df = pd.DataFrame.from_records(evt_keys, columns = ['step1', 'step2','copyNo', 'pid'])
    ...
    energy, evt_copyNo_Ke_dose_df = calculateDose(eventEdep, chromatinVolume, keys_df)   

    print("calculateDose Done")

    print("loading Direct tree") #this can take long, so just checking when it's done
    ...
    events_strand_copy_direct_df = calcDirectDamage(cumulatedEnergyDep_df, fEMinDamage, fEMaxDamage)
    if not len(events_strand_copy_direct_df):
        raise ValueError("no events in direct list.")
    
    indirect = ufile["output/Indirect"]
    events_strand_copy_indirect_df = calcIndirectDamage(indirect, probIndirect, T0, T1, keys_df, out_path)
    ...
    if separate_r:
        ...
        for r in ranges_radii:
            ...
            events_direct = events_strand_copy_direct_df.loc[events_strand_copy_direct_df['copyNo'].apply(check_copynop)]
            events_indirect = events_strand_copy_indirect_df.loc[events_strand_copy_indirect_df['copyNo'].apply(check_copynop)]
            events_tot = evt_copyNo_Ke_dose_df.loc[evt_copyNo_Ke_dose_df['copyNo'].apply(check_copynop)]
            
            # get all events (direct+indirect)
            # events_tot_ids = events_tot['step1'].to_numpy()
            
            unique_pids = list(set(np.concatenate([np.unique(events_direct['pid']), np.unique(events_indirect['pid'])])))
            unique_pids.append(-1)
            #run clustering on selected events
            for particle in unique_pids:
                if particle==-1:
                    events_direct_part = events_direct[events_direct['pid']!=-1]
                    events_indirect_part = events_indirect[events_indirect['pid']!=-1]
                else:
                    events_direct_part = events_direct[events_direct['pid']==particle]
                    events_indirect_part = events_indirect[events_indirect['pid']==particle]

                numEvts = list(set(np.concatenate([events_direct_part['step1'], events_indirect_part['step1']])))
                tempResults_part = clustering(numEvts, np.array(events_direct_part['step1']), np.array(events_direct_part['copy']), np.array(events_direct_part['strand']), 
                                        np.array(events_indirect_part['step1']), np.array(events_indirect_part['copy']), np.array(events_indirect_part['strand']))
                #print(tempResults)
                clusteringResults = np.asarray(tempResults_part[0], dtype=int)  # strand break number results
                clusterSize = np.asarray(tempResults_part[1])  # DSB cluster size, cluster of size 1-10
                ...
                if len(clusteringResults):
                    ...
                    text_SB = np.zeros(shape = (eventsWithClusteringResults.shape[0], 12), dtype=int)
                    text_SB[:] = clusteringResults[:, 1:]
                    
                    text_DSB = np.zeros(shape=(eventsWithClusteringResults.shape[0], 50), dtype=int)
                    text_DSB[:] = clusterSize[:,1:]
                    
                    particle_name_map = {v: k for k, v in particleMap.items()}
                    # print(particle, unique_pids)
                    outfile_SB = out_path+f"_{particle_name_map[particle]}"+f"_{r}.csv" 
                    ...
                    with open(outfile_SB, "w") as f:
                        ...
                        f.write("{},{},{},{},{},{},{},{},{}\n".format(filename_DNA, np.mean(energy), LET, len(numEvts), chromatinVolume, numBP, sugarFname, gitHash, clusteringGitHash))
                        ...
                        for i, event in enumerate(sorted(eventsWithClusteringResults)):
                            if particle==-1:
                                ...
                                f.write("{},{},{},{},{}".format(
                                    event, 
                                    events_tot['dose'][(events_tot['step1']==event)&(events_tot['pid']!=particle)].sum(), 
                                    particle, # evt_copyNo_Ke_dose_df['pid'][evt_copyNo_Ke_dose_df['step1']==event], 
                                    events_tot['edep_MeV'][(events_tot['step1']==event)&(events_tot['pid']!=particle)].sum(), 
                    ...
                    outfile_DSB = out_path+f"_{particle_name_map[particle]}"+f"_DSB_{r}.csv"
                    with open(outfile_DSB, "w") as f:
                        ...
                        for i, event in enumerate(sorted(eventsWithClusteringResults)):
                            if particle==-1:
                                f.write("{},{},{},{}".format(
                                    event, events_tot['dose'][(events_tot['step1']==event)&(events_tot['pid']!=particle)].sum(),
                                    particle, 
